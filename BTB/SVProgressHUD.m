...
+ (SVProgressHUD*)sharedView {
    ...
    dispatch_once(&once,
                  ^ { sharedView = [[SVProgressHUD alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
                      //sharedView.transform = CGAffineTransformMakeRotation(M_PI*(90)/180.0);
                  });
    return sharedView;
}


+(void)setViewStyle:(int)tag {
    if (0==tag) {
        
    }else if(1==tag){
        [SVProgressHUD sharedView].transform = CGAffineTransformMakeRotation(M_PI*(90)/180.0);//旋转
    }else{
